        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-12 m-auto table" id="table" style="margin-top: 10%;">
            <table class="table table-bordered table-hovered table-striped" id="productTable">
                ...
                foreach($products as $product) : ?>
                    ...
                    <tr>
                        <td> <?php echo $product['ind_nombre_disciplina']; ?> </td>
                        <td> <button type="button" class="edit" VALUE="<?php echo $product['pk_num_disciplina']; ?>" class="btn btn-link btn-lg"  data-toggle="modal" data-target="#exampleModal">
            ...
            <div class="row">
                ...
                <button type="button" id="nuevo" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#exampleModal">Nuevo Disciplina</button>
            </div>
        </div>
        </div>
                    ...
                    <div class="form">
                        <div class="row ">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-8">
                                    <input type="hidden" name="pk_num_disciplina" id="pk_num_disciplina">
                                    <input id="fname" name="name" type="text" placeholder="Nombre de la Disciplina" class="form-control">
                                </div>
                            </div>
...
<script>
    $(document).ready( function () {
        $('#nuevo').click( function () {
            $('#fname').val('');
            $('#pk_num_disciplina').val('');
        });

        $('#guardar').click( function () {
            var equipo= $('#fname').val();
            var id= $('#pk_num_disciplina').val();
            if (id!=""){
                var caso = "editarDisciplina";
                var datos= {'disciplina':equipo, 'pk_num_disciplina': id,'caso': caso};
            }else {
                var caso="guardarDisciplina";
                var datos= {'disciplina':equipo, 'caso': caso};
        ...
        $('.edit').click( function () {
            ...
            var caso="verDisciplina";
            $.ajax({
                type: "POST",
                url: '../controlador/enrutador.php',
                data: {'pk_num_equipo':pk_num_equipo, 'caso': caso},//capturo array
                success: function(data){
                    var equipo= $('#fname').val(data.dato.ind_nombre_disciplina);
                    var pk_num_equipo= $('#pk_num_disciplina').val(data.dato.pk_num_disciplina);
