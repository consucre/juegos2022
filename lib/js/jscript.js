...
function totalLotes(registros, rows, limit) {
	...
	if (registros) {
		if (limit==0) {
			btPrimera.src="../imagenes/primera_d.png"; btPrimera.onclick="";
			btAnterior.src="../imagenes/anterior_d.png"; btAnterior.onclick="";
		} else {
		...
		if ((registros<=MAXLIMIT) || ((limit+rows)==registros) || (limit==registros)) {
			btSiguiente.src="../imagenes/proxima_d.png"; btSiguiente.onclick="";
			btUltima.src="../imagenes/ultima_d.png"; btUltima.onclick="";
		}
		else {
...
function msjError(error) {
	switch (error) {
		case 1000: alert("¡Debe llenar los campos obligatorios!"); break;
		case 1010: alert("¡Debe seleccionar un registro!"); break;
		case 1020: alert("¡No puede ingresar el mismo equipo mas de una vez en un encuentro!"); break;
...
function validarPreguntas(accion) {	
	var numero=document.getElementById("numero").value; numero=numero.trim();
	var pregunta=document.getElementById("pregunta").value; pregunta=pregunta.trim();
	var a=document.getElementById("a").value; a=a.trim();
	var b=document.getElementById("b").value; b=b.trim();
	var c=document.getElementById("c").value; c=c.trim();
	var d=document.getElementById("d").value; d=d.trim();
	var respuesta=document.getElementById("respuesta").value; respuesta=respuesta.trim();
	var puntos=document.getElementById("puntos").value; puntos=puntos.trim();
	var ronda=document.getElementById("ronda").value; ronda=ronda.trim();
...
function validarEquipos(accion) {	
	var numero=document.getElementById("numero").value; numero=numero.trim();
	var equipo=document.getElementById("equipo").value; equipo=equipo.trim();
	if (equipo=="0") msjError(1000);
		...
		ajax.open("POST", "../lib/fphp.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("maestro=EQUIPOS&accion="+accion+"&numero="+numero+"&equipo="+equipo);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4)	{
				if (ajax.responseText=="EXITO") {
...
function validarEncuentros(accion) {	
	var numero=document.getElementById("encuentro").value; numero=numero.trim();
	var equipo1=document.getElementById("equipo1").value; equipo1=equipo1.trim();
	var equipo2=document.getElementById("equipo2").value; equipo2=equipo2.trim();
	var equipo3=document.getElementById("equipo3").value; equipo3=equipo3.trim();
	var ronda=document.getElementById("ronda").value; ronda=ronda.trim();
...
function validarParametros(accion) {
	var parametro=document.getElementById("parametro").value; parametro=parametro.trim();
	var descripcion=document.getElementById("descripcion").value; descripcion=descripcion.trim();
	var valor=document.getElementById("valor").value; valor=valor.trim();
...
function procesarRespuesta() {
	...
	if (turno==1) { var tequipo="equipo1"; var puntos="divPuntos1"; var turnos="divTurnos1"; }
	if (turno==2) { var tequipo="equipo2"; var puntos="divPuntos2"; var turnos="divTurnos2"; }
	if (turno==3) { var tequipo="equipo3"; var puntos="divPuntos3"; var turnos="divTurnos3"; }
	var equipo=document.getElementById(tequipo).value;
...
function finEncuentro() {
	...
	if (equipo3!=0) var puntos3=document.getElementById("divPuntos3").innerHTML; else { var puntos3=0; equipo3=0; }
	...
	if (status==1 && respondido==0) alert ("¡Acción inválida! \nPregunta en proceso...");
		...
		if (x) {
			...
			ajax.onreadystatechange=function() {
				...
				if (ajax.readyState==4)	{
					if (ajax.responseText=="EXITO") {
						location.href="http://localhost/reto_2/archivos/jugar.php?accion=LISTAR&limit=0";
					} else msjError(ajax.responseText);
				}	
			}
		}
